		...
		if(bcxx_wait_cmd2("+NSOSTR:",TIMEOUT_15S) == RECEIVED)
		{
			if(bcxx_wait_cmd2("+NSONMI:",TIMEOUT_15S) == RECEIVED)
			{
				...
				if(bcxx_wait_cmd2("OK",TIMEOUT_2S) == RECEIVED)
				{
					...
					recv_len = get_str1((unsigned char *)bcxx_rx_cmd_buf, ",", 4, ",", 5, http_buf);
					...
					if(recv_len != 0)
					{
						...
						pos_s = MyStrstr(data_buf, "GMT\r\n\r\n", recv_len, 7);
						
						if(pos_s != 0xFFFF)
						{
							recv_len = recv_len - pos_s - 7;

							memcpy(outbuf,&data_buf[pos_s + 7],recv_len);
