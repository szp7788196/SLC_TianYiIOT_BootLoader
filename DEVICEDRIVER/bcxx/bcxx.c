...
void bcxx_hard_init(void)
{
	...
	if(BOARD_TYPE == 'A' || BOARD_TYPE == 'C' || BOARD_TYPE == 'E')
	{
...
void BCXX_RST_HIGH(void)
{
	GPIO_SetBits(GPIOC,GPIO_Pin_2);
}

void BCXX_RST_LOW(void)
{
	GPIO_ResetBits(GPIOC,GPIO_Pin_2);
}

void BCXX_PWREN_HIGH(void)
{
	if(BOARD_TYPE == 'A' || BOARD_TYPE == 'C' || BOARD_TYPE == 'E')
	{
...
unsigned short bcxx_set_AT_NSOSD(unsigned char socket, unsigned int len,char *inbuf,unsigned char *outbuf)
{
	unsigned short ret = 0;
	unsigned short recv_len = 0;
	unsigned short i = 0;
	unsigned char http_buf[1358];
	unsigned char data_buf[679];
	char *msg = NULL;
	char tmp[10];
	...
	if(SendCmd(cmd_tx_buf, "+NSONMI:", 100,0,TIMEOUT_60S) == 1)
	{
		memset(cmd_tx_buf,0,256);
		
		sprintf(cmd_tx_buf,"AT+NSORF=%d,%d\r\n",socket,1358);
		if(SendCmd(cmd_tx_buf, "OK", 100,0,TIMEOUT_5S) == 1)
		{
			memset(http_buf,0,1358);
			memset(data_buf,0,679);

			recv_len = get_str1((unsigned char *)result_ptr->data, ",", 4, ",", 5, http_buf);
			...
			if(recv_len != 0)
			{
				...
				StrToHex(data_buf, (char *)http_buf, recv_len);
				
				msg = strstr((char *)data_buf,"Content-Length: ");
				...
				recv_len = nbiot_atoi(tmp,strlen(tmp));
				
				msg = msg + 4;
				
				memcpy(outbuf,msg,recv_len);
